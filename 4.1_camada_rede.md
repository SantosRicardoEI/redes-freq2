...
Analogia dos slides (viagem de carro):

- **Roteamento** = planear toda a viagem da origem até ao destino (escolher o caminho no mapa).
- **Encaminhamento** = tomar a direção certa num único cruzamento/rotunda ao longo dessa viagem.
...
**Duas abordagens** para o plano de controlo (slides):

- **Algoritmos de roteamento tradicionais** — implementados *dentro* dos routers (cada router participa no cálculo). É o caso clássico, estudado nesta aula.
- **SDN (Software-Defined Networking)** — o controlo é feito por um **controlador remoto**, implementado em servidores, separado dos routers. Os routers ficam só com o plano dos dados.
...
## 18. Notas de apoio para perguntas

Na Internet, a camada de rede aparece como um conjunto de peças:

- **IP** — formato dos datagramas, endereçamento e encaminhamento básico.
- **ICMP** — mensagens de erro e sinalização da camada de rede.
- **Protocolos/algoritmos de roteamento** — calculam caminhos e preenchem tabelas de encaminhamento.
- **SDN** — alternativa em que um controlador remoto calcula/programa as regras usadas pelos routers/switches.

As garantias de um modelo de serviço podem ser pensadas em dois níveis:

- **Datagramas individuais** — entrega garantida, entrega até certo tempo, ausência de perda.
- **Fluxos de datagramas** — ordem, largura de banda mínima, atraso/jitter controlado entre pacotes.

A Internet best effort não dá estas garantias fortes. Mesmo assim, muitas aplicações em tempo real funcionam de forma aceitável porque usam adaptação de taxa, buffers de reprodução, CDNs, retransmissões seletivas quando possível, redundância e porque a capacidade das redes tem aumentado. O custo é que a aplicação tem de lidar com perdas, atraso e variação de atraso.
